...
from core.model.net import Net_vqa, NET_rubi
...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        loss_qm = torch.nn.CrossEntropyLoss(reduction='sum').cuda()
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            for step, (
                ...
                for accu_step in range(self.__C.GRAD_ACCU_STEPS):
                    ...
                    pred_rubi, pred_q = net(
                        sub_img_feat_iter,
                        sub_ques_ix_iter
                    )

                    loss_rubi = loss_fn(pred_rubi, sub_ans_iter)
                    loss_q = loss_qm(pred_q,sub_ans_iter)
                    loss = loss_q + loss_rubi

                    # precision = precision_score(sub_ans_iter.cpu().data.numpy(), pred_q.cpu().data.numpy(), average = 'macro')
                    # recall = recall_score(sub_ans_iter.cpu().data.numpy(), pred_q.cpu().data.numpy(), average = 'macro')
                    # f1 = f1_score(sub_ans_iter.cpu().data.numpy(), pred_q.cpu().data.numpy(), average = 'macro')
                    ...
                    loss.backward()
                    loss_sum = loss_sum + loss.cpu().data.numpy() * self.__C.GRAD_ACCU_STEPS
                    # precision_sum = precision_sum + precision.cpu().data.numpy() * self.__C.GRAD_ACCU_STEPS
                    # recall_sum = recall_sum + recall.cpu().data.numpy() * self.__C.GRAD_ACCU_STEPS
                    # f1_sum = f1_sum + f1.cpu().data.numpy() * self.__C.GRAD_ACCU_STEPS
