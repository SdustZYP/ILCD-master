...
class DataSet(Data.Dataset):
    def __init__(self, __C):
        ...
        self.stat_ques_list = \
            json.load(open(__C.QUESTION_PATH['train'], 'r',encoding='utf-8', errors='ignore'))['questions'] + \
            json.load(open(__C.QUESTION_PATH['val'], 'r',encoding='utf-8', errors='ignore'))['questions'] + \
            json.load(open(__C.QUESTION_PATH['test'], 'r',encoding='utf-8', errors='ignore'))['questions'] + \
            json.load(open(__C.QUESTION_PATH['vg'], 'r',encoding='utf-8', errors='ignore'))['questions']
        ...
        split_list = __C.SPLIT[__C.RUN_MODE].split('+')
        for split in split_list:
            self.ques_list += json.load(open(__C.QUESTION_PATH[split], 'r',encoding='utf-8', errors='ignore'))['questions']
            if __C.RUN_MODE in ['train']:
                self.ans_list += json.load(open(__C.ANSWER_PATH[split], 'r',encoding='utf-8', errors='ignore'))['annotations']
